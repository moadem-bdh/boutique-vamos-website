...
export default function AboutSection() {
  ...
  return (
    <section id="about" className=" gap-12 lg:gap-0 w-full  min-h-229 flex flex-col items-center justify-between py-16 bg-white  px-4 md:px-6 lg:px-10 xl:px-20 ">
      ...
      <StaggerContainer staggerDelay={0.12} className="grid grid-cols-[repeat(1,minmax(280px,360px))] sm:grid-cols-[repeat(2,minmax(280px,360px))] lg:grid-cols-4 xl:grid-cols-[repeat(4,minmax(240px,1fr))] items-center gap-4 justify-evenly w-full">
      ...
      <MotionWrapper variants={fadeUp} delay={0.1} className="flex flex-col items-center lg:items-start w-full gap-4">
        <h2 className=" text-3xl md:text-[34px] lg:text-[40px] font-bold text-black font-rale ">
          {aboutData.socialMediaTitle[language]}
        </h2>
        <StaggerContainer staggerDelay={0.08} className="flex flex-row w-full flex-wrap justify-between sm:gap-x-4 md:gap-x-8  gap-y-2">
