...
import MotionWrapper, { fadeUp, fadeLeft, fadeRight, fadeIn } from "@/components/MotionWrapper";
...
export default function HeroSection() {
  ...
  return (
    <section className="relative items-center lg:items-start flex flex-col px-4 md:px-6 lg:px-10 xl:px-20 justify-center gap-6 lg:gap-10 xl:gap-16  w-full h-screen min-h-180 z-1">
      ...
      <MotionWrapper variants={fadeIn} duration={0.8} className="block lg:hidden -mt-10 md:mt-12">
        <Image         
          src="/assets/logo.svg"
          alt="Foot Ball image"
          width={170}
      ...
      <div className=" lg:mt-10 xl:mt-16 flex items-center lg:items-start md:gap-2.5 lg:1.5 flex-col w-full lg:w-max ">
        ...
        <MotionWrapper variants={fadeIn} delay={0.4} duration={0.5} className="w-90/100 lg:w-full">
          <hr className=" w-full border-white border-3 mt-8 lg:mt-8 xl:mt-10 rounded  " />
        </MotionWrapper>
      </div>

      <StaggerContainer staggerDelay={0.08} className=" flex md:mt-6 lg:mt-0  flex-row w-full max-w-134 lg:max-w-none lg:w-130 xl:w-148 justify-between ">
      ...
      <MotionWrapper variants={fadeRight} delay={0.2} duration={0.8} className="hidden lg:block absolute  min-h-140 xl:min-h-160 w-max h-[85vh] xl:h-[90vh] right-10 xl:right-20 bottom-0">
        <Image
